"""
research-figure-skill :: export_figure.py
=========================================
...
def export_figure(
    ...
    for fmt in formats:
        if fmt in {"jpg", "jpeg"}:
            print(f"[research-figure-skill] WARNING: skipping {fmt} — "
        ...
        saved.append(path)
        print(f"[research-figure-skill] wrote {path}")
...
def _grayscale_from(fig, basename: str, dpi: int) -> str | None:
    ...
    try:
        ...
        print("[research-figure-skill] Pillow not available; "
    ...
    print(f"[research-figure-skill] wrote {gray_path} (grayscale preview)")
    return gray_path
...
def _cli() -> int:
    p = argparse.ArgumentParser(description="research-figure-skill figure exporter")
